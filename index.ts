...
server.tool(
  ...
  async ({ project, businessRule, changeDescription }) => {
    ...
    // === 6. conventions.md — Preserve existing content, only update auto-detected section ===
    const conventionsPath = path.join(memoryDir, "conventions.md");
    const AUTO_SECTION_MARKER = "<!-- AUTO-DETECTED BY CODEATLAS - DO NOT EDIT BELOW THIS LINE -->";
    ...
    if (fs.existsSync(conventionsPath)) {
      // Preserve existing content, only replace auto-detected section
      const existing = fs.readFileSync(conventionsPath, "utf-8");
      const markerIndex = existing.indexOf(AUTO_SECTION_MARKER);
      if (markerIndex !== -1) {
        // Replace everything from marker onwards
        const preserved = existing.substring(0, markerIndex).trimEnd();
        fs.writeFileSync(conventionsPath, preserved + "\n\n" + autoDetectedSection + "\n");
      } else {
        // No marker found — append auto section to existing content
        fs.writeFileSync(conventionsPath, existing.trimEnd() + "\n\n" + autoDetectedSection + "\n");
      }
    } else {
      // First time — create fresh with header + auto section
      const freshContent = [
        `# Conventions — ${loaded.projectName}`,
        `> Auto-generated by CodeAtlas MCP`,
        `> You can add notes, error patterns, fix guides above the auto-detected section — they will be preserved.`,
        "",
      ].join("\n");
      fs.writeFileSync(conventionsPath, freshContent + "\n" + autoDetectedSection + "\n");
    }
